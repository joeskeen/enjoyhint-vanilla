...
export const Konva = KonvaMod.default ?? KonvaMod;
...
export class EnjoyHintImpl {
    ...
    init() {
        ...
        const shape = new Konva.Shape({
            ...
            sceneFunc: function (context) {
                ...
                roundRect(x, y, this.attrs.width, this.attrs.height, this.attrs.radius, ctx);
                ctx.fill();
        ...
        const resizeHandler = () => {
            ...
            document.querySelector(".enjoy_hint_label")?.remove();
            ...
            const boundingClientRect = document
                .querySelector(this.stepData.enjoyHintElementSelector)
                .getBoundingClientRect();
            this.shape.attrs.centerX = Math.round(boundingClientRect.left + boundingClientRect.width / 2);
            this.shape.attrs.centerY = Math.round(boundingClientRect.top + boundingClientRect.height / 2);
    ...
    renderAfterResize() {
        ...
        document.querySelector(".enjoyhint_next_btn").style.visibility = "visible";
        document.querySelector(".enjoyhint_prev_btn").style.visibility = "visible";
        document.querySelector(".enjoyhint_skip_btn").style.visibility = "visible";
    }
    show() {
    ...
    renderArrow(data) {
        ...
        setTimeout(() => {
            ...
            document.querySelector(".enjoyhint_svg").appendChild(path);
    ...
    renderLabelWithShape(data, customBtnProps) {
        this.stepData = data;
        this.customBtnProps = customBtnProps;
        const dialog = this.findParentDialog(document.querySelector(this.stepData.enjoyHintElementSelector));
        if (dialog != null) {
        ...
        let sidesPos = { top: 0, bottom: 0, left: 0, right: 0 };
        switch (shapeType) {
            case "circle":
        ...
        const labelVerticalSpaceRequired = window.innerHeight <= 670
        ...
        for (let i = 0; i < areasPriority.length; i++) {
            ...
            if (area.width > labelHorizontalSpaceRequired &&
                area.height > labelVerticalSpaceRequired) {
                labelHorizontalSide = name;
            }
        }
        const dataWidthSize = data.shape === "circle"
            ? data.radius * 2
            : data.width
                ? data.width
                : data.radius * 2;
        const dataHeightSize = data.shape === "circle"
            ? data.radius * 2
            : data.height
                ? data.height
                : data.radius * 2;
        ...
        const centralVerPosition = window.innerHeight / 2 - labelVerticalSpaceRequired / 2 + 20;
        let labelX = 0;
        let labelY = 0;
        let xTo = 0;
        let yTo = 0;
        ...
        setTimeout(() => {
            const summaryButtonWidth = this.nextButton.offsetWidth +
